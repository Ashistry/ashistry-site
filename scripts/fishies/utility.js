export class Utility {
    ...
    static meanRGB(RGB1, RGB2) {
        if (!Utility.validateRGB(RGB1) || !Utility.validateRGB(RGB2)) {
            throw new Error(`one of the provided RGB values did not pass the RGB validator! check the log for details.`);
        }
        const result = [
            (RGB1[0] + RGB2[0]) / 2,
            (RGB1[1] + RGB2[1]) / 2,
        ...
        if (!Utility.validateRGB(result)) {
            throw new Error(`${result} did not pass the RGB validator! check log for details.`);
        }
        else {
            return result;
        }
